(ns aoc2018.d02-test
  (:require [aoc2018.d02 :refer [checksum solve-2]]
...
(deftest part-1
  (testing "example"
    (let [box-ids ["abcdef" "bababc" "abbcde" "abcccd" "aabcdd" "abcdee" "ababab"]]
      (is (= 12 (checksum box-ids)))))

  (testing "input"
    (let [box-ids (str/split-lines input)]
      (is (= 9139 (checksum box-ids))))))


(deftest part-2
  (testing "example"
    (let [box-ids ["abcde" "fghij" "klmno" "pqrst" "fguij" "axcye" "wvxyz"]]
      (is (= "fgij" (solve-2 box-ids)))))

  (testing "input"
    (let [box-ids (str/split-lines input)]
      (is (= "uqcidadzwtnhsljvxyobmkfyr" (solve-2 box-ids))))))
